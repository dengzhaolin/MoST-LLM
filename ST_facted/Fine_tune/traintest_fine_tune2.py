...
from model_tune1 import Prediction



from model_tune_2 import LLMFineTuner


from ST_facted.signal_facted.SignalFacted_model import SignalFactedVAE
from ST_facted.multi_facted.MultiFactedVAE import MultiFacedVAE
...
def predictModel(fine_tuner,pre_trained_vae,prediction_head, seq, dynamic_batch=True):
    ...
    for i in gen_batch(seq, min(args.batch_size, len(seq[0])), dynamic_batch=dynamic_batch):
        ...
        with torch.no_grad():
            batch_data = test_seq_th.squeeze(-1).permute(0, 3, 1, 2)
...
def traintest_model(dataset):
    ...
    optimizer = optim.Adam([
        ...
        {'params': prediction_head.parameters(), 'lr': 0.005},
    ...
    for epoch in range(nb_epoch):
        ...
        for j, x_batch in enumerate(gen_batch(dataset.get_data('train'), args.batch_size, dynamic_batch=True, shuffle=True)):
            ...
            batch_data = x.squeeze(-1).permute(0,3,1,2)
            ...
            loss = criterion(pred, y)#+0.1*aux_loss
...
def get_model():
    ...
    #prediction_head=DriftAwareMoE().to(device)
    prediction_head=Prediction(384,16)



    pre_trained_multi_vae = MultiFacedVAE(4,16, 4, 192).to(device)

    pre_trained_model_path = "/data/dzl2023/MoSSL/ST_facted/multi_facted/MODEL/MultiFactedVAE/MoSSL_NYC_num_comp4_hc48_l4_his16_pred3_v0.h5"
    ...
    # pred_trained_signal_vae = SignalFactedVAE(pre_trained_multi_vae.encoder, args.hidden_channels).to(device)
    #
    # pre_trained_signal_path="/data/dzl2023/MoSSL/ST_facted/MODEL/MoSSL_NYC_num_comp4_hc48_l4_his16_pred3_v0.h5"
    #
    ...
    return fine_tuner,prediction_head, pre_trained_multi_vae#pred_trained_signal_vae
...
parser.add_argument('--cuda', default=3, type=int, help='cuda name')
