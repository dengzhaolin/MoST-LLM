...
from lib.utils import data_gen, gen_batch, get_metric,gen_batch1

from model_tune1 import Prediction,ModalMoE

from model_tune1 import LLMFineTuner
...
data_embedding="/data/dzl2023/MoSSL/module/fine_tune_merge/Embeddings"



train_embedding=h5py.File(f"{data_embedding}/train/train_embedding.h5")['embeddings'][:]
print(train_embedding.shape)

print(time.strftime('%Y-%m-%d', time.localtime()))

test_embedding=h5py.File(f"{data_embedding}/test/test_embedding.h5")['embeddings'][:]

val_embedding=h5py.File(f"{data_embedding}/val/val_embedding.h5")['embeddings'][:]


def predictModel(fine_tuner,pre_trained_vae,prediction_head, seq,embedding, dynamic_batch=True,cross_model= None):
    fine_tuner.eval()
    prediction_head.eval()
    pre_trained_vae.eval()
    #cross_model.eval()
    pred_list = []
    for j,(x_batch ,prompt_embedding) in enumerate(gen_batch1(seq, min(args.batch_size, len(seq[0])),embedding, dynamic_batch=dynamic_batch)) :
        # Note: use np.copy() to avoid the modification of source data.
        test_seq = np.copy(x_batch[:, 0:args.input_length, :, :, :])
        step_list = []
        test_seq_th = torch.tensor(test_seq, dtype=torch.float32).to(device)

        prompt_embedding = torch.tensor(prompt_embedding, dtype=torch.float32).permute(0, 2, 1).to(device)

        with torch.no_grad():
            batch_data = test_seq_th.squeeze(-1).permute(0, 3, 2, 1)

            batch_size=batch_data.shape[0]

            input_embedding = pre_trained_vae.module.encoder(batch_data)

            #rep = pre_trained_vae.module.most_encoder(input_embedding)

            rep = input_embedding.reshape(batch_size, args.num_nodes, -1)

            #rep = torch.cat([prompt_embedding,rep], dim=-1)

            #pad_mask = torch.zeros(batch_size, args.num_nodes, dtype=torch.bool).to(device)

            #rep,_ = cross_model(rep, prompt_embedding,key_padding_mask=pad_mask, return_attn=True)

            #rep = fine_tuner.module.embedding_to_hidden(rep)

            rep = fine_tuner(rep,prompt_embedding).reshape(batch_size, -1, args.num_modals, args.num_nodes).unsqueeze(-1)

            pred = prediction_head(rep).permute(0, 1, 3, 2, 4)
...
def modelInference(fine_tuner,pre_trained_vae,prediction_head, inputs,cross_model= None):
    ...
    # evaluation
    y_val, len_val = predictModel(fine_tuner,pre_trained_vae,prediction_head, x_val,val_embedding,cross_model= None)
    mae_val, rmse_val = get_metric(x_val[0:len_val, args.input_length:args.horizon + args.input_length, :, :, :],
                                   y_val[:, :, :, :, :], x_stats)
    # test
    y_test, len_test = predictModel(fine_tuner,pre_trained_vae,prediction_head,x_test,test_embedding,cross_model= None)
...
def traintest_model(dataset):
    ...
    #cross_model=MultiModalFusion(192, 1536,1536, mode="gated", d_model=1536+192, n_heads=8).to(device)

    save_dir = "./model/prompt"
    ...
    for epoch in range(nb_epoch):
        ...
        #cross_model.train()
        losses = []
        for j, (x_batch ,prompt_embedding)in enumerate(gen_batch1(dataset.get_data('train'), args.batch_size,train_embedding, dynamic_batch=True, shuffle=True)):
            ...
            batch_data = x.squeeze(-1).permute(0,3,2,1)
           ...
           #rep = fine_tuner.module.embedding_to_hidden(rep)
            ...
            rep = fine_tuner(rep,prompt_embedding)

            rep=rep.reshape(batch_size, -1, args.num_modals, args.num_nodes).unsqueeze(-1)
        ...
        lr_scheduler.step()
        end_time = time.time()
        mae_val, rmse_val, mae_test, rmse_test = modelInference(fine_tuner,pre_trained_vae,prediction_head, dataset,cross_model=None)
        print('=' * 80)
...
def get_model():
    ...
    pred_trained_signal_vae = SignalFactedVAE(pre_trained_multi_vae.encoder,4, args.hidden_channels).to(device)
