...
class MultiFacedEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        temporal_encoded = self.temporal_encoder(encoded)

        ST_encoded=torch.cat([spatial_encoded,temporal_encoded],dim=1)

        mu = self.fc_mu(ST_encoded)

        return mu

class MultiFacedDecoder(nn.Module):
    def __init__(self, modality_channel, hidden_dim=192, out_channels=4):
        ...
        self.decoder_pred = nn.Sequential(
            nn.ReLU(),
            nn.Conv2d(hidden_dim, hidden_dim//2, kernel_size=(5,1)),
            nn.ReLU(),
            nn.Conv2d(hidden_dim//2, hidden_dim //4,kernel_size=(5,1)),
            nn.ReLU(),
            nn.Conv2d(hidden_dim //4, out_channels , kernel_size=(6,1)),

        )
        # self.decoder_pred = nn.Sequential(
        #     nn.ReLU(),
        #     nn.Conv2d(hidden_dim, hidden_dim // 2, kernel_size=(1, 1)),
        #     nn.ReLU(),
        #     nn.Conv2d(hidden_dim // 2, hidden_dim // 4, kernel_size=(1, 1)),
        #     nn.ReLU(),
        #     nn.Conv2d(hidden_dim // 4, out_channels, kernel_size=(1, 1)),
        #
        # )
...
class MultiFacedVAE(nn.Module):
    def __init__(self,modality_channel, in_channels=4, hidden_dim=192):
        super(MultiFacedVAE, self).__init__()
        self.encoder = MultiFacedEncoder(in_channels, hidden_dim)
        self.decoder = MultiFacedDecoder(modality_channel, hidden_dim, out_channels=modality_channel)
