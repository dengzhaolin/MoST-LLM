...
## Residual Block
########################################
class ResidualBlock_revice(nn.Module):
    def __init__(self, num_modals, num_nodes, channels, dilation, kernel_size):
        super(ResidualBlock_revice, self).__init__()
        self.num_modals = num_modals
        self.num_nodes = num_nodes
        self.channels = channels
        self.dilation = dilation
        ...
        # Temporal Convolution
        self.filter_convs = nn.ConvTranspose3d(in_channels=self.num * self.channels,
                                               out_channels=self.channels,
                                               kernel_size=(1, 1, self.kernel_size),
                                               dilation=(1, 1, self.dilation))
        self.gate_convs = nn.ConvTranspose3d(in_channels=self.num * self.channels,
                                             out_channels=self.channels,
                                             kernel_size=(1, 1, self.kernel_size),
                                             dilation=(1, 1, self.dilation))
        self.residual_convs = nn.Conv3d(in_channels=self.channels, out_channels=self.channels, kernel_size=(1, 1, 1))
...
class ST_Decoder(nn.Module):
    def __init__(self, layers, num_modals, num_nodes, channels, kernel_size):
        ...
        dilation = 1
        for i in range(self.layers):
            self.residualblocks.append(ResidualBlock_revice(num_modals, num_nodes, channels, dilation, kernel_size))
            dilation *= 2
...
class SignalFactedDecoder(nn.Module):
    ...
    def __init__(self, channels,input_len,pred_len):
        ...
        self.decoder_recover = nn.Sequential(ST_Decoder(layers=4, num_modals=4, num_nodes=98, channels=channels, kernel_size=2),
                                             nn.Conv3d(in_channels=channels, out_channels=channels // 2,
                                                       kernel_size=(1, 1, 1)),
                                             nn.ReLU(),
                                             nn.Conv3d(in_channels=channels // 2, out_channels=1,
                                                       kernel_size=(1, 1, 1)))
        ...
        self.decoder_pred = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv3d(in_channels=channels // 4, out_channels=pred_len, kernel_size=(1, 1, 1)),
        )
...
class SignalFactedVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return pred.permute(0,1,3,2,4),reconstructed.permute(0,4,3,2,1) , multi_facted_embedding.squeeze(-1).permute(0,3,2,1)
...
#
# model = SignalFactedVAE(vae.encoder,4,48)
#
