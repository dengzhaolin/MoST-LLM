...
def get_model():
    ...
    pre_trained_vae = MultiFacedVAE(4, 16,4, 192).to(device)
...
def traintest_model(dataset):
    ...
    os.makedirs('MODEL/MultiFactedVAE_Signal', exist_ok=True)
    save_model_path = os.path.join('MODEL/MultiFactedVAE_Signal', '{}.h5'.format(file_name))
    print('=' * 10)
    ...
    for epoch in range(nb_epoch):  
        ...
        for j, x_batch in enumerate(gen_batch(dataset.get_data('train'), args.batch_size, dynamic_batch=True, shuffle=True)):
            ...
            if epoch_within_100 < temporal_epochs:
                masked_data, mask = apply_mask_single(x, mask_type="temporal", temporal_hours=temporal_hours,
                ...
                masked_data, mask = apply_mask_single(x, mask_type="spatial", temporal_hours=temporal_hours,
                                               spatial_points=spatial_points, return_mask=True)
