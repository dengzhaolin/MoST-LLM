...
def traintest_model(dataset):
    model = get_model()    
    file_name = "MultiFactedVAE_{}".format(args.data_name)
    os.makedirs('MODEL/MultiFactedVAE', exist_ok=True)
    save_model_path = os.path.join('MODEL/MultiFactedVAE', '{}.h5'.format(file_name))
    print('=' * 10)
...
if args.data_name == 'NYC':
    ...
    args.num_nodes = 98
    args.num_modals = 4
    dataset = data_gen('/data/dzl2023/MoSSL/data/BJ/BJ.h5', (n_train, n_val, n_test), args.num_nodes, args.input_length + args.horizon, args.num_modals, day_slot=48)
