...
from SignalFacted_model_AUTO import SignalFactedVAE
from ST_facted.multi_facted.MultiFacted_model import MultiFacedVAE
from lib.utils import data_gen, gen_batch, get_metric, get_metric_save
...
def get_model():
    ...
    pre_trained_vae = MultiFacedVAE(4, 4, 192).to(device)
    ...
    pre_trained_vae.load_state_dict(pretrained_dict, strict=False)

    vae = SignalFactedVAE(pre_trained_vae.encoder,4, args.hidden_channels).to(device)
...
def predictModel(model, seq, dynamic_batch=True):
    ...
    for i in gen_batch(seq, min(args.batch_size, len(seq[0])), dynamic_batch=dynamic_batch):
        ...
        pred,rec,_= model(test_seq_th)
        pred = rec.data.cpu().numpy()
...
def modelInference(model, inputs):
    ...
    mae_val, rmse_val= get_metric_save(x_val[0:len_val, :args.input_length , :, :, :], y_val[:, :, :, :, :], x_stats)
    # test
    y_test, len_test = predictModel(model, x_test)
    mae_test, rmse_test = get_metric_save(x_test[0:len_test, :args.input_length , :, :, :], y_test[:, :, :, :, :], x_stats)
    return mae_val, rmse_val, mae_test, rmse_test


def traintest_model(dataset):
    model = get_model()
    file_name = "SignalFactedVAE_{}".format(args.data_name)
    os.makedirs('MODEL/SignalFactedVAE', exist_ok=True)
    save_model_path = os.path.join('MODEL/SignalFactedVAE', '{}.h5'.format(file_name))
    print('=' * 10)
    ...
    #             masked_data, mask = apply_mask(x, mask_type="temporal", temporal_hours=temporal_hours,
    #                                            spatial_points=spatial_points, return_mask=True)
    #
    #             # if epoch_within_100 == 0:
    #             #     print("Entering Temporal Masking Strategy")
    #
    #         elif epoch_within_100 < temporal_epochs + spatial_epochs:
    #             masked_data, mask = apply_mask(x, mask_type="spatial", temporal_hours=temporal_hours,
    #                                            spatial_points=spatial_points, return_mask=True)
    #             # if epoch_within_100 == 25:
    #             #     print("Entering Spatial Masking Strategy")
    #
    #         else:
    #             masked_data, mask = apply_mask(x, mask_type="agnostic", temporal_hours=temporal_hours,
    #                                            spatial_points=spatial_points, return_mask=True)
    #             # if epoch_within_100 == 50:
    #             #     print("Entering Agnostic Masking Strategy")
    #
    #         pred, rec, _ = model(masked_data)
    #         reg_loss = criterion(pred, y)
    #         rec_loss = vae_masked_loss_function(rec, x, mask=mask)
    #
    #         #print(" Reg Loss: {}, Rec Loss: {}".format(reg_loss.item(), rec_loss.item()))
...
parser.add_argument("--epsilon", default=1e-4, type=float, help="optimizer epsilon")
...
if args.data_name == 'NYC':
    ...
    args.num_nodes = 98
    args.num_modals = 4
    dataset = data_gen('/data/dzl2023/MoSSL/data/NYC.h5', (n_train, n_val, n_test), args.num_nodes, args.input_length + args.horizon, args.num_modals, day_slot=48)
