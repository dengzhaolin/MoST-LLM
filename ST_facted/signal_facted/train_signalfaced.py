...
from SignalFactedModel_Full import SignalFactedVAE
import argparse
from lib.utils import data_gen, gen_batch
...
def multifaced_train(dataset, epochs=1000, lr=0.001, device=None):
    #pre_trained_vae = MultiFacedVAE(modality_channel=4).to(device)

    pre_trained_vae =MultiFacedVAE(4, 4,192).to(device)

    pre_trained_model_path = "/data/dzl2023/MoSSL/ST_facted/multi_facted/model/multi_modality_mask_8_50_epoch_300_modile.pth"
    ...
    vae = SignalFactedVAE(pre_trained_vae.encoder,args.hidden_channels).to(device)
    ...
    for epoch in range(epochs):
        ...
        for j, x_batch in enumerate(
            ...
            if epoch_within_100 < temporal_epochs:
                masked_data, mask = apply_mask_single(batch_data, mask_type="temporal", temporal_hours=temporal_hours,
                ...
                masked_data, mask = apply_mask_single(batch_data, mask_type="spatial", temporal_hours=temporal_hours,
                                               spatial_points=spatial_points, return_mask=True)
                # if epoch_within_100 == 25:
                #     print("Entering Spatial Masking Strategy")

            else:
                masked_data, mask = apply_mask_single(batch_data, mask_type="agnostic", temporal_hours=temporal_hours,
                                               spatial_points=spatial_points, return_mask=True)
        ...
        if (epoch + 1) % 100 == 0:
            torch.save(vae.state_dict(), os.path.join(save_dir, f'signal_modality_mask_{temporal_hours}_{spatial_points}_epoch_{epoch + 1}_modile.pth'))
