...
def get_model():
    ...
    pre_trained_model_path = "/data/dzl2023/MoSSL/ST_facted/Hyper/MODEL/MultiFactedVAE_Hyper/MultiFactedVAE_NYC_0.75.h5"
...
def traintest_model(dataset):
    model = get_model()
    file_name = "SignalFactedVAE_{}_{}".format(args.data_name,0.75)
    os.makedirs('MODEL/SignalFactedVAE_Hyper', exist_ok=True)
    save_model_path = os.path.join('MODEL/SignalFactedVAE_Hyper', '{}.h5'.format(file_name))
    print('=' * 10)
...
parser.add_argument('--cuda', default = 5, type=int, help='cuda name')
