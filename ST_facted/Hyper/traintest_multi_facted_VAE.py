...
#from MultiFactedVAE import MultiFacedVAE
...
def traintest_model(dataset):
    ...
    file_name = "MultiFactedVAE_{}_{}".format(args.data_name, 0.75)

    os.makedirs('MODEL/MultiFactedVAE_Hyper', exist_ok=True)
    save_model_path = os.path.join('MODEL/MultiFactedVAE_Hyper', '{}.h5'.format(file_name))
    print('=' * 10)
    ...
    for epoch in range(nb_epoch):  
        ...
        for j, x_batch in enumerate(gen_batch(dataset.get_data('train'), args.batch_size, dynamic_batch=True, shuffle=True)):
            ...
            if epoch_within_100 < temporal_epochs:
                ...
                masked_data, mask = apply_rand_mask(x, mask_type="agnostic", temporal_hours=temporal_hours,
                                               spatial_points=spatial_points, return_mask=True)
