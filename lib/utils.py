...
def gen_batch1(inputs, batch_size,embedding, dynamic_batch=False, shuffle=False):

    embedding=embedding[:len(inputs)]
    assert  len(inputs)==len(embedding)
    len_inputs = len(inputs)
    ...
    for start_idx in range(0, len_inputs, batch_size):
        ...
        yield inputs[slide], embedding[slide]
...
def get_metric_save(y, y_, x_stats):
    y = z_inverse(y, x_stats['mean'], x_stats['std'])
