...
class DMSTGCN(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3,
                 out_dim=16*4, residual_channels=16, dilation_channels=16, end_channels=512,
                 kernel_size=2, blocks=4, layers=2, days=288, dims=40, order=2, in_dim=4, normalization="batch"):
        ...
        self.end_conv_1 = nn.Conv2d(in_channels=608,
    ...
    def forward(self, inputs):
        ...
        inputs = inputs.squeeze(-1).permute(0, 3, 2, 1)

        self.nodevec_p1 = nn.Parameter(torch.randn(inputs.shape[0], self.dims).to(inputs.device), requires_grad=True)
        self.nodevec_a1 = nn.Parameter(torch.randn(inputs.shape[0], self.dims).to(inputs.device), requires_grad=True)
        self.nodevec_a2p1 = nn.Parameter(torch.randn(inputs.shape[0], self.dims).to(inputs.device), requires_grad=True)
        """
        input: (B, F, N, T)
        """
        ...
        x_a = self.start_conv_a(xo)
        skip = 0

        # dynamic graph construction
        adp = self.dgconstruct(self.nodevec_p1, self.nodevec_p2, self.nodevec_p3, self.nodevec_pk)
        adp_a = self.dgconstruct(self.nodevec_a1, self.nodevec_a2, self.nodevec_a3, self.nodevec_ak)
        adp_a2p = self.dgconstruct(self.nodevec_a2p1, self.nodevec_a2p2, self.nodevec_a2p3, self.nodevec_a2pk)
        ...
        x = self.end_conv_2(x)
        x=x.reshape(x.shape[0],in_len,-1,x.shape[2],x.shape[-1]).permute(0,1,3,2,4)
        return x


# x=torch.randn(64,4,98,16)
#
# model=DMSTGCN(torch.device('cpu'),98)
# y=model(x)
# print(y.shape)
