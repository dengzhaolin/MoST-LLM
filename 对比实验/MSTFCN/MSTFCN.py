...
class MSTFCN(nn.Module):
    def __init__(self, device, num_nodes, gcn_depth=2, dropout=0.0, input_dim=[2, 2], output_dim=[2, 2],
                 window=16, horizon=16, subgraph_size=20, node_dim=40, tanhalpha=3, propalpha=0.00,
                 layers=2, residual_channels=32, conv_channels=32, skip_channels=64, end_channels=64, cross=2, temporal_func='MLP', add_time=True):
    ...
    def forward(self, input, **kwargs):
        ...
        b = [torch.randn(bs, self.residual_channels, self.n_nodes, self.window).to(input.device) for _ in range(self.n_mix)]  # (n_mix, bs, res_channel, n_nodes, window)
...
# model=MSTFCN(torch.device('cpu'),98)
# y=model(x)
# print(y.shape)
