...
class IDGCN_Tree(nn.Module):
    def __init__(
        ...
        self.memory1 = nn.Parameter(torch.randn(channels, num_nodes, 8))
        self.memory2 = nn.Parameter(torch.randn(channels, num_nodes, 4))
        self.memory3 = nn.Parameter(torch.randn(channels, num_nodes, 4))
...
class STIDGCN(nn.Module):
    def __init__(
        ...
        self.regression_layer = nn.Conv2d(
            channels, self.output_len*4, kernel_size=(1, self.output_len)
        )
    ...
    def forward(self, input):
        ...
        prediction = prediction.reshape(input.shape[0], self.output_len,-1, self.num_nodes, 1).permute(0,1,3,2,4)
        return prediction

#
# model =STIDGCN(torch.device("cpu"),4,98,128,288)
#
