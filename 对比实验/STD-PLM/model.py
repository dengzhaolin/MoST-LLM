...
from utils import norm_Adj, lap_eig, topological_sort
from sandglassAttn import SAG
import numpy as np
...
class STALLM(nn.Module):
    def __init__(self,basemodel,device,sample_len=16, output_len=16,
                 input_dim=4 , output_dim=4 ,
                  node_emb_dim =128, sag_dim=128, sag_tokens=128,
                 adj_mx = None, use_node_embedding = True,
    ...
    def forward(self,input:torch.FloatTensor,prompt_prefix=None):

        mask= torch.ones_like(input.squeeze(-1))
        other_loss = []

        x= input.squeeze(-1).permute(0,2,3,1).reshape(input.shape[0],input.shape[2],-1)



        B,N,TF = x.shape #(Batch,N,T*features)
        # emb of time
        te  = torch.randn(B,input.shape[2],self.tim_dim).to(x.device)
        # emb of nodes
        if self.use_node_embedding:
        ...
        out= out.reshape(B,N,self.output_len,self.output_dim).permute(0,2,1,3).unsqueeze(-1)
    ...
    def setadj(self,adj_mx,dis_mx):
        ...
        self.adj_mx = torch.tensor(adj_mx).to(self.device)
        self.dis_mx = torch.tensor(dis_mx).to(self.device)
        self.d_mx = self.adj_mx.sum(dim=1)
        N = self.adj_mx.shape[0]
        self.alpha = torch.tensor([1.05] * N).to(self.device) + torch.softmax(self.d_mx,dim=0)*5
