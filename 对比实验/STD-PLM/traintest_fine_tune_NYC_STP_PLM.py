...
def traintest_model(dataset):
    ...
    save_dir = f"./model/{model._get_name()}_{args.data_name}"
...
def get_model():
    graph = np.array(torch.ones(args.num_nodes,args.num_nodes))

    graph = np.array(graph, dtype=np.float32)

    # graph =torch.tensor(graph, dtype=torch.float32).to(device)

    base_model = GPT2(lora=False, ln_grad=False)

    model = STALLM(basemodel=base_model,device=device, adj_mx=graph).to(device)
