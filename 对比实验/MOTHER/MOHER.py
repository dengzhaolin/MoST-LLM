...
class MOHER(nn.Module):
    ...
    def __init__(self, device, adj_mx, num_nodes, window=16, horizon=16, input_dim=[2,2], output_dim=[2,2], gamma=0.1, beta=0.8, subgraph_size=30,
                 static_feat=None, n_heads=4, n_layers=3, hidden_dim=128, dropout=0.0, summarize=False, add_time=False):
    ...
    def forward(self, input, **kwargs):
        ...
        for i in range(self.n_mix):
            ...
            x[i] = self.time_encoder(x[i], torch.randn(bs,self.window, self.num_nodes,2 )) if self.add_time else x[i]  # (bs, dim, n_nodes, 1)
            x[i] = F.relu(self.end_conv[i](x[i]).squeeze(dim=-1))  # (bs, horizon * output_dim, n_nodes)
            x[i] = x[i].reshape(bs, self.horizon, self.output_dim[i], self.num_nodes).permute(0, 1, 3, 2)

        x = torch.cat(x, -1)
        return x.unsqueeze(dim=-1)
# x=torch.randn(64,16,98,4,1)
# adj=torch.randn(98,98)
# model=MOHER(torch.device('cpu'),adj,98)
# y=model(x)
# print(y.shape)