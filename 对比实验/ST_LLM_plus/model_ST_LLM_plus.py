...
class PFA(nn.Module):
    def __init__(self, device="cuda:0", gpt_layers=6, U=1, dropout_rate=0.0):
        ...
        self.gpt2 = GPT2Model.from_pretrained("/data/dzl2023/LLM-Factory/GPT2", attn_implementation="eager",
...
class ST_LLM(nn.Module):
    ...
    def forward(self, history_data):
        ...
        data = input_data.permute(0, 3, 2, 1)
        B, T, S, F = data.shape
        # print(data.shape) #[64, 12, 250, 3]
        
        #Temporal Embedding
        tem_emb = torch.randn(B,self.gpt_channel,S,1).to(data.device)
        ...
        prediction = outputs.reshape(B, self.output_len, self.input_dim, S, 1).permute(0, 1, 3, 2,
                                                                                                           4)
